...
from src.train.trainer_cv import Trainer
...
# ------------------
# Build model (stub hook)
# ------------------
def build_model(cfg: Dict[str, Any], enc_in: int) -> nn.Module:
    """
    Bạn sẽ implement sau. Ở đây chỉ tạo hook theo model_name.
    """
    name = cfg["model_name"]
    if name == "DeepTokenEEG":
        # TODO: bạn sẽ thay bằng model thật của bạn
        from src.models.model import Model  # fallback nếu bạn đang có class Model cũ
        return Model(
            enc_in=int(enc_in),
...
def main():
    ...
    for band in cfg["band_set"]:
        ...
        X, enc_in = standardize_X_to_NTC(X, max_channels=int(cfg.get("max_channels", 64)))
        ...
        for fold_idx in range(int(cfg["folds"])):
            ...
            # model/trainer
            model = build_model(cfg, enc_in=enc_in).to(device)
